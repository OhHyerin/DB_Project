...
<body>
	...
	<div id="body">
		...
		<?
			...
			//세션아이디로 u_prm얻어오기
			$q_getUprm = "SELECT ID FROM USER_T WHERE ID='".$_SESSION['userid']."';";
			$r_getUprm = mysqli_query($conn,$q_getUprm);
			$row_getUprm = mysqli_fetch_array($r_getUprm);
			$userPrm = $row_getUprm['ID'];

			$nDate = date('Y-m-d');//NOW(TODAY)

			//포스트한 글이 있는지 없는지 검사
			$q_chkIf = "SELECT S_PRM FROM POST_T WHERE ID='".$userPrm."'";
			...
			$q_selF = "SELECT S_PRM,S_TITLE,S_GOALSUM,S_DEADLINE FROM POST_T WHERE ID='".$userPrm."'";
			...
			while($row_selF=mysqli_fetch_array($r_selF)){
				...
				//공연 당 후원한 회원정보
				$q_infoUser = "SELECT ID,U_PHONE,d_money FROM USER_T AS U JOIN D_INFO_T AS D ON U.ID = D.ID WHERE D.S_PRM='".$show_prm."';";
