...
<?
		...
		$q_getU = "SELECT ID FROM USER_T WHERE ID='".$userid."';";
		$r_getU = mysqli_query($conn,$q_getU);
		$row_getU = mysqli_fetch_array($r_getU);
		$uprm = $row_getU['ID'];

		$q_getInfo = "SELECT D_PRM, S_TITLE, D_MONEY,D_DATE FROM D_INFO_T JOIN POST_T ON D_INFO_T.S_PRM=POST_T.S_PRM WHERE D_INFO_T.ID='".$uprm."' AND D_INFO_T.S_PRM='".$sprm."';";
