...
<body>
	...
	<?
			...
			//u_prm으로 id 찾기
			$q_selID = "SELECT ID FROM USER_T WHERE ID='".$_SESSION['userid']."'";
			...
			$q_selD = "SELECT S_PRM,D_MONEY  FROM D_INFO_T WHERE ID='".$userPrm."'";
			...
			$q_Sinfo = "SELECT P.S_PRM as SPRM,S_TITLE,S_GOALSUM,D.D_MONEY as D_MONEY FROM POST_T AS P LEFT JOIN D_INFO_T AS D ON P.S_PRM = D.S_PRM WHERE D.ID='".$userPrm."'";
			...
			while($row_Sinfo=mysqli_fetch_array($r_Sinfo)){
				...
				$q_cnt = "SELECT ID, COUNT(D_PRM) AS CntS FROM D_INFO_T WHERE ID='".$userPrm."' GROUP BY ID";
